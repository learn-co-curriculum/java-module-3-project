...
Create a ticket package in the `src/main/java` directory called "ticket". 

- This is where we will implement the ticket feature. 

Create an abstract class called `Ticket`.

- Add an `int` instance variable called `ticketNumber` since every ticket
  will have a ticket number associated with it. 
- Create a constructor that takes in a `ticketNumber` and initializes the
  instance variable. 
- Use IntelliJ to generate a getter method for the instance variable. 
- Create an abstract method called `getPrice()` that takes no parameters and
  returns a `double`. 
- Add the following code to override the `toString()` method:
...
Modify the `purchaseTicket()` method in the `Concert` class.

- Rework this method to return an `int` value instead of a `boolean` value.
- Modify the method to match the description:
  - If there are still tickets available, return a positive integer with
...
- Run the `ConcertTest` and ensure all the tests still pass.

Modify the `purchaseTicket` method in the `ConcertService` class.

- Have the `purchaseTicket` method take in two more parameters:
  - A `boolean` to determine if the ticket buyer is a student.
  - A date-time object to determine the date the ticket buyer would like to
    buy the ticket.
- Implement the following logic:
  - If the `purchaseDate` is after the `concertDate`, then the ticket is
    ...
    - Make sure to print that ticket is unavailable.
  - If there are no tickets available, print the ticket is unavailable.
  - If there are tickets still available, check to see if the ticket buyer
    is a student. If yes, print the `StudentTicket` and that a ticket has
    been purchased.
  - If there are tickets still available, and the ticket buyer is not a
    ...
    - Consider the
      ...
      to help compare date-time objects.
  - If there are still tickets available, and the ticket buyer is not a
    ...
    purchased.
    - Hint: There are a few ways you can go about this problem. Consider
    ...
    - Print the `AdvanceTicket` and that a ticket has been purchased.
- Update the `ConcertServiceTest`:
...
- Run the `ConcertServiceTest` and ensure all the tests still pass.

Modify the `Driver` class.

- Create a new static void method called `purchaseTicket()`.
  - Prompt the user for the performer's name.
  - Prompt the user for boolean to determine if the ticket buyer is a
    student.
  - Prompt the user for the date they wish to purchase the ticket using the
    form: MM/dd/uuuu.
    - Example: 03/17/2023
    - Use the `ResolverStyle.STRICT` to force the user to enter a valid
      date.
- Call the `ConcertService` method, `purchaseTicket()` given the new
  parameters.
  - Hint: Take a look at how we created the `addConcert()` method in the
    driver class from the previous task.
  - In the `switch` statement, change `case "p":` to use the new static
    method you created.
- If we run the driver class now, an expected output could be this:
